...
import {initialState} from './state/post-reducer';
import PostDetailsContainer from './post-details';
...
describe('Post Details', () => {
  describe('Container', () => {
    ...
    beforeEach(() => {
      const postState = initialState.set('post', post);
      store = mockStore({
        location: {
          pathname: '/posts/1',
        },
    ...
    it('should render', () => {
      const wrapper = shallow(<TestContainer><PostDetailsContainer /></TestContainer>);

      expect(wrapper.isEmptyRender()).toBeFalsy();
    });

    it('should Post Details receive props', () => {
      const match = {
        params: {
          postId: '1',
        },
      ...
      const wrapper = mount(<TestContainer store={store}>
        <PostDetailsContainer match={match} />
      </TestContainer>); // eslint-disable-line
      const actual = wrapper.find(Details).props();
      expect({...post, isLoading: false}).toEqual(actual);
    });
    it('should redner Post Details', () => {
      const match = {
        params: {
          postId: '1',
        },
      ...
      const actual = wrapper.find(Details);
